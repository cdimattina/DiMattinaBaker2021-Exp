...
stepParams.ori  = -45;
stepParams.phase = 0;
stepParams.dLset = 3000;
[lumDiscInc,~,~] = makeStepLSBStim(stepParams);
stepParams.ori   = +45;
[lumDiscCon0,~,~] = makeStepLSBStim(stepParams);
stepParams.phase = 180;
[lumDiscCon180,~,~] = makeStepLSBStim(stepParams);
...
%     
%     IText   = I;
      IDiscInc   = gray  + inc*lumDiscInc;
      IDiscCon0  = gray  + inc*lumDiscCon0;
      IDiscCon180 = gray + inc*lumDiscCon180;
    ...
    subplot(2,2,2);
    image(uint8(I + (IDiscInc-gray)));
    axis square off; 
    
    subplot(2,2,3);
    image(uint8(I + (IDiscCon0-gray)));
    axis square off; 

    subplot(2,2,4);
    image(uint8(I + (IDiscCon180-gray)));
    axis square off; 
