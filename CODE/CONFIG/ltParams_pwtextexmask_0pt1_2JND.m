...
% Get the JND for luminance disc and PW for luminance texture for  A = 0pt1 
load(sprintf('../../ANALYSIS/RESULTS/gaussBlobRandPropLoCon_%s.mat',subj)); 

JND75PU   = threshstruct.JND75PU;
JNDunits  = 2;
pU        = JNDunits*JND75PU; 
pW        = 0.5*(1 + pU); 

stim.category               = 'pwtextTextMask';  
 
% Pedestal is a gaussian blob
...
stim.pedestal.blob.vary     = 'pW';
stim.pedestal.blob.pW       = pW;
stim.pedestal.blob.dens     = 0.5;
stim.pedestal.blob.maxlum   = 0.1; 
stim.pedestal.blob.nPattSide = 32;
stim.pedestal.blob.mpSize   = 8; 
stim.pedestal.blob.jitter   = 4;
stim.pedestal.blob.sigma    = 2;
stim.pedestal.blob.gridspace = 20;
stim.pedestal.blob.dodots   = 0;
...
stim.blob.dens      = [0.5];
stim.blob.maxlum    = [0.1];  % Keep blob between 0.7 and 0.3
