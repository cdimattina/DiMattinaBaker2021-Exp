...
task.nReps          = 20;                                                   % <<< SET number of repetitions
...
stim.category   = 'gabStep';          
stim.modDepth   = 100; 
stim.mpsize     = 32;
stim.npatt      = [512];                     % Level for density: one level only
                                              % config file

stim.orB        = 0;                          % Fixed orientation
stim.orALevs    = [0, 5, 10, 15, 30, 45];     % Variable orientation
task.nOrLev     = length(stim.orALevs);       % Number of orientation levels

stim.lumConAddLevs  = [0 1/2 1 2 4 8];              % Levels for step luminance edge
stim.pWALevs        = [];                         % Used for texture luminance edge
