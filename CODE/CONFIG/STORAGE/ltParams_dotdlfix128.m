...
stim.category       = 'gaussBlobDLFix';  
stim.method         = 'staircase';                                                                               
stim.stairNDown     = 2;        

% Details of the gaussBlob stimulus
stim.blob.vary      = 'nCL';   % 'pW' or 'dens'
stim.blob.nWL       = 6;
stim.blob.nCL       = [18 12 6 4 0];

% stim.blob.pW        = [0.55  0.6  0.65  0.7 0.75 0.8];
% stim.blob.dens      = [0.5];
...
stim.blob.jitter    = 2; 
stim.blob.sigma     = 1; 
stim.blob.gridspace = 10; 
stim.blob.dodots    = 1;    % 0 - gaussians, 1 - polka-dots
...
elseif(strcmp(stim.blob.vary,'nCL'))
    stim.nLevels    = length(stim.blob.nCL);
end
...
stim.size           = 128;          % must be <= screen height, in pixels     % << SET stimulus size
