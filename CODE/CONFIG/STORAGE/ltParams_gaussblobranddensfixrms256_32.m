...
stim.blob.vary      = 'nPattSide';   % 'pW' or 'dens'
stim.blob.pW        = [0.65];
stim.blob.dens      = [0.5];
stim.blob.maxlum    = [0.25];  % Keep blob between 0.7 and 0.3

stim.blob.nPattSide = [16 24 32 48 64];         % Must divide 4 
stim.blob.mpSize    = 8;
stim.blob.jitter    = 4; 
stim.blob.sigma     = 2; 
...
elseif(strcmp(stim.blob.vary,'nPattSide'))
    stim.nLevels    = length(stim.blob.nPattSide);
elseif(strcmp(stim.blob.vary,'maxlum'))
    stim.nLevels    = length(stim.blob.maxlum); 
end

% Stimulus normalization methods

% Fix overall RMS contrast to desired level
stim.RMSfix         = 1;        %  
stim.RMSlev         = 0.02;     % Desired RMS contrast level

% Fix DL value
stim.DLfix          = 0; 
stim.DLlev          = 350;      %       

% Check to make sure one is not trying to fix both RMS and DL
% at the same time. These are different operations...
if(stim.RMSfix && stim.DLfix)
   error('cannot fix DL and RMS at same time!');  
end
